      ...
      <div className="max-w-[1320px] mx-auto">
        ...
        <div className="grid grid-cols-3 gap-8 max-lg:grid-cols-2 max-md:grid-cols-1 relative">
          {STEPS.map((step, idx) => (
            <div
              key={step.num}
              className={`group bg-white rounded-[28px] py-10 px-8 relative border border-ink/[0.05] transition-all duration-[400ms] ease-soft hover:-translate-y-1.5 hover:shadow-lift hover:border-tangerine-soft ${
                idx === 1 ? "max-lg:mt-0 mt-8" : ""
              } ${idx === 2 ? "max-lg:mt-0 mt-4" : ""}`}
            >
              <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-tangerine-mist text-tangerine-deep font-display italic text-[28px] mb-7 transition-all duration-[350ms] ease-spring group-hover:bg-tangerine group-hover:text-white group-hover:scale-110 group-hover:-rotate-[5deg]">
                {step.num}
              </div>
