// Session 65 — POST /api/admin/donors/[id]/reject
//
// Body: { reason: string }   (min 10, max 1000 chars)
//
// Sets og_admin_approval_status='rejected' + stamps og_admin_approved_at.
// Reason captured in audit_log metadata (no donor-row column for it).
// Audited as admin_rejected_donor.
//
// Status mapping:
//   200 ok            — { donorId, rejectedAt }
//   400 bad_request   (malformed body)
//   400 invalid_state (reason too short / long)
//   401 unauthorized
//   404 not_found
//   500 server_error
...
export async function POST(
  ...
  try {
    const result = await rejectDonor(id, session.userId, reason, req);
    ...
    if (err instanceof DonorWriteFailedError) {
      console.error(
        "[/api/admin/donors/reject] write_failed",
        err.message,
      );
      return NextResponse.json(
    ...
    console.error(
      "[/api/admin/donors/reject] server_error",
