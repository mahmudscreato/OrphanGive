// Session 52b — Admin moment rejection endpoint.
...
export async function POST(
  ...
  try {
    const result = await rejectMoment(
      id,
      adminSession.userId,
      parsed.data.reason,
    );
    if (result.uploaderId) {
      ...
      await notify({
        ...
        type: "admin_rejected_moment",
        title: `Moment rejected for ${childLabel}`,
    ...
    console.error(
      "[/api/admin/moments/reject] server_error",
