...
import { listAuditEventsForChild } from "@/lib/di-audit";
...
import { HistoryPanel } from "@/components/di/HistoryPanel";
...
export default async function DiChildDetailPage({
  ...
  // Five parallel reads (Session 46: history added). Each scope-
  // guards independently (belt-and-braces — the parent already 404'd
  // if out of scope, so any null returns here would be a race
  // condition). All read functions return [] (or empty list) on
  // miss, never throw, so partial failures degrade gracefully
  // (e.g. moments unreachable doesn't break the Profile tab).
  const [sponsorships, moments, reports, deliveries, history] =
    await Promise.all([
      ...
      listAuditEventsForChild(id, session.userId),
    ]);
  ...
  return (
    <div className="px-5 md:px-10 lg:px-12 py-6 md:py-10 max-w-5xl mx-auto">
      ...
      <ChildDetailTabs
        ...
        historyContent={<HistoryPanel events={history} />}
      />
    </div>
  );
}
